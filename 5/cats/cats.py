...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def check(p):
        for p_word in remove_punctuation(p).lower().split():
            for t_word in topic:
                if (p_word == t_word):
                    return True
        return False
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    correct, num_t, num_s = 0, len(typed_words), len(source_words)
    short, long = num_t, num_s
    ...
    if num_t > num_s:
        short, long = num_s, num_t 
    for i in range(short):
        if (typed_words[i] == source_words[i]):
            correct += 1
    return 100 * correct / num_t
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    closest_w, smallest_diff = typed_word, limit + 1
    for word in word_list:
        new_diff = diff_function(typed_word, word, limit)
        if new_diff <= limit and new_diff < smallest_diff:
            closest_w, smallest_diff = word, new_diff
        if word == typed_word:
            return typed_word
    return closest_w
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    tl, sl = len(typed), len(source)
    shortest = sl
    if tl < sl:
        shortest = tl

    def helper(i, num_fixes):
        if typed[i] != source[i]:
            num_fixes += 1
        if i == 0 or num_fixes > limit:
            return num_fixes
        else:
            return helper(i-1, num_fixes)
    return helper(shortest - 1, abs(tl - sl))
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    if start == "" or goal == "":  # Fill in the condition
        ...
        return 1
        # END
    elif len(start) > 0 and len(goal) > 0 and start[0] == goal[0]:
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    # example output: ['collar': [1, 3], 'plush': [3], [4]]
    # return words[word_ind]: [[li[word_ind + 1] - li[word_ind] for li in times_per_player[p]] for p in times_per_player] for word_ind in words
    # END PROBLEM 9
